...
export class UserPlanService {
    ...
    create(userPlan: IUserPlan): Observable<EntityResponseType> {
        return this.http.post<IUserPlan>(this.resourceUrl, userPlan, { observe: 'response' });
    }

    update(userPlan: IUserPlan): Observable<EntityResponseType> {
        return this.http.put<IUserPlan>(this.resourceUrl, userPlan, { observe: 'response' });
    }

    find(id: number): Observable<EntityResponseType> {
        return this.http.get<IUserPlan>(`${this.resourceUrl}/${id}`, { observe: 'response' });
    }

    query(req?: any): Observable<EntityArrayResponseType> {
        const options = createRequestOption(req);
        return this.http.get<IUserPlan[]>(this.resourceUrl, { params: options, observe: 'response' });
    }
